...
export interface BodyParamsArray<T extends Record<string, unknown>[] = Record<string, unknown>[]> {
  body: T | T[];
}

export interface QueryParams<T extends Record<string, unknown> = Record<string, unknown>> {
...
export type EventParams = BodyParams | BodyParamsArray | QueryParams | PathParams;

export type Handler<P extends EventParams, isProtected extends boolean = true> = (
  event: Omit<APIGatewayProxyEvent, 'body' | 'pathParameters' | 'queryStringParameters'> & {
  ...
  context: isProtected extends true ? Context : Context,
